%步行速度分辨方法
    ...
    clear;
    %数据预处理
    DATA=xlsread('C:\Users\201810\Desktop\dedyAxmz\数据\100Hz数据\上楼\上楼女一组.xlsx',1);%体育场
    %data=DATA(:,1:3).*9.8;
    data=DATA(:,3:8);
    ...
    left_peaks_locs=[];
    for n=1:33
            [pks,locs] = findpeaks(ay(120+window*(n-1):170+window*(n-1)),'MinPeakHeight',0.35,'MinPeakDistance',7);
            locs=locs+119+window*(n-1);
            left_peaks_locs=[left_peaks_locs;locs];
    end
    window=145;
    for n=1:13
            [pks,locs] = findpeaks(ay(5045+window*(n-1):5084+window*(n-1)),'MinPeakHeight',0.35,'MinPeakDistance',7);
            locs=locs+5044+window*(n-1);
            left_peaks_locs=[left_peaks_locs;locs];
    end
    window=140;
    for n=1:7
            [pks,locs] = findpeaks(ay(6950+window*(n-1):7000+window*(n-1)),'MinPeakHeight',0.375,'MinPeakDistance',4);
            locs=locs+6949+window*(n-1);
            left_peaks_locs=[left_peaks_locs;locs];
    end
peaks=[];
peaks=[left_peaks_locs];
for i=2:length(left_peaks_locs)
    if left_peaks_locs(i)-left_peaks_locs(i-1)<30
        peaks(i)=0;
    end
end
peaks(find(peaks==0))=[];
left_peaks_locs=peaks;
% plot(1:length(ay),ay,peaks_locs,ay(1:length(peaks_locs)),'o');
...
%% %谷底点
[pks,locs] = findpeaks(-ay,'MinPeakHeight',-0.22,'MinPeakDistance',20);
valley_locs=[locs];
for i=2:length(locs)
...
valley_locs(find(valley_locs==0|valley_locs==8023))=[];
%% 右波峰点
[pks,locs] = findpeaks(ay,'MinPeakHeight',0.23,'MinPeakDistance',3);
% findpeaks(ay,'MinPeakHeight',0.23,'MinPeakDistance',3);
...
% %倒三角
findpeaks(ay,'MinPeakHeight',0.35,'MinPeakDistance',30);
findpeaks(-ay,'MinPeakHeight',-0.22,'MinPeakDistance',20);
