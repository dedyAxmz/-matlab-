...
clear;
%数据预处理
DATA=xlsread('C:\Users\201810\Desktop\dedyAxmz\数据\100Hz数据\下楼\下楼男一组.xlsx',1);%体育场
%data=DATA(:,1:3).*9.8;
data=DATA(:,2:7);
predata=data;
% [predata] =boy_datapreprocesssing(data);
%归一化
...
%局部峰值位置统计
%波峰
[pks,locs] = findpeaks(ay,'MinPeakHeight',0.28,'MinPeakDistance',9);
peaks_locs=[locs];
findpeaks(ay,'MinPeakHeight',0.28,'MinPeakDistance',9);
%% 波谷
[pks,locs] = findpeaks(-ay,'MinPeakHeight',-0.23,'MinPeakDistance',25);
findpeaks(-ay,'MinPeakHeight',-0.23,'MinPeakDistance',25);
valley_locs=[];
for a=1:14
    if mod(a,2)==0 %偶数
         valley_locs=[valley_locs;locs(a)];
    end
end
for a=15:29
    if mod(a,2)==1 %奇数
...
valley_locs=[valley_locs;2822;6278;6390;6489]
...
% %倒三角
findpeaks(ay,'MinPeakHeight',0.36,'MinPeakDistance',16);
