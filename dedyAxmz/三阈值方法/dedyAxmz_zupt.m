...
%section=[0
%% 方差阈值
v_threshold=[0.000402,0.00045,0.00045,0.00045];
var_4sec_0_1=[];
for q=1:4
  [var_sec_0_1]=var_threshold(acceleration((1+(q-1)*900):1:(900+(q-1)*900),1),v_threshold(1,q));
  var_4sec_0_1=[var_4sec_0_1;var_sec_0_1];
end
% x=1:1000;
% plot(x,Var1(1:1000).*10,x,acceleration(1:1000),x,var_test0_1(1:1000));
%% 时间间隔(<11)阈值，误判检测
...
x=1:1000;
plot(x,gyro_xyzfan_Var(1:1000).*10,x,gyro_xyzfan(1:1000),x,gyro_xyzfan_Var0_1(1:1000));
%% 时间间隔阈值，误判检测
...
x=1:1000;
plot(x,gyro_xyzfan_Var(1:1000).*10,x,gyro_xyzfan(1:1000),x,var_gyro_0_1(1:1000));
%步态结果区分画图（检测结果0:1000）对不对
x=1:1000;
plot(x,acceleration(1:1000),x,gyro_xyzfan(1:1000),x,var_gyro_0_1(1:1000));
plot(x,ay(1:1000),x,var_gyro_0_1(1:1000))